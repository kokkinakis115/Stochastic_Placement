# -*- coding: utf-8 -*-
"""
Created on Wed Jan 31 15:21:36 2024
...
def produce_volatility_dict(volatility_arr):
...
def produce_workload(intensity, vol):
...
workload_volatility_array = [calculate_volatility(workload) for workload in workloads]
workload_averages_array = [statistics.median(workload) for workload in workloads]

vol_dict = produce_volatility_dict(workload_volatility_array)
...
class Microservice: #microservice class, time_series is created upon initialization based on given intensity of application
    ...
    def _create_usage1(self, duration_in_hours, intensity, volatility):
        ...
        wl = create_wl(sigma=sigma, mu=mu, lambd=lambd, users=200, duration_in_hours=duration_in_hours)
    ...
    def _create_usage2(self, intensity, volatility):
        
        self.usage_time_series = produce_workload(intensity=intensity, vol=volatility)
        return
...
class BSM_predictor:
    ...
    def _compute_options(self, workload, starting_strike):
        ...
        for timeslot_num in range(num_of_time_slots): #compute loop for each timeslot (1h)
            ...
            prediction = min(2.5*spot_price, compute_strike_price2(spot=spot_price, vol=vol, texp=texp, intr=self.intr, delta=self.threshold, cp=cp))
    ...
    def _predict_next_timeslot(self, microservice, prediction_timeslot_num):
        ...
        prediction = min(2*spot_price, compute_strike_price2(spot=spot_price, vol=vol, texp=texp, intr=self.intr, delta=self.threshold, cp=cp))        
...
class Simulation:
    ...
    def _produce_initial_scheme(self):
        initial_scheme = []
    ...
    def _run_simulation(self, prediction_model):
        ...
        initial_assignment_scheme = self._produce_initial_scheme()
        
        # run allocation algorithm to produce initial_assignment_scheme # !!!
        
        initial_allocation_scheme = create_random_scheme(self)
